...
```bash
# 第一步：诊断——看看哪些 Skill 还在占坑、哪些该走了
/ctx-check

# 第二步：根据诊断结果决定要不要回收

# 第三步：回收——清理已完成任务的 Skill 内容
/ctx-clean
```
...
| 命令 | 本质 | 做什么 |
|------|------|--------|
| `/ctx-check` | 上下文诊断 | 让 Claude 分析自己的对话历史，识别哪些 Skill 被加载了、任务完成了没、值不值得回收 |
| `/ctx-clean` | 智能回收 | 对 `/compact` 的封装——带着"保留清单"去压缩，优先回收已完成 Skill 的完整内容 |
